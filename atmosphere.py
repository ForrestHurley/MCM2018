...
def mirror(vectors,normals):
    reflect= np.subtract(vectors, 2*np.sum(vectors*normals,axis=1)*normals)
    return reflect
...
def points_of_incidence2(center, radius, line_points, directions):
    ...
    return (np.add(line_points,distance1*directions),np.add(line_points,distance2*directions))
...
def unitize(vectors):
    return np.divide(vectors,np.linalg.norm(vectors,axis=1))
...
def reflect_rays_sphere(center, radius, points, directions):
    ...
    incidence_points=np.where(boolean_table, incidence_points[1],incidence_points[0])            
...
def main():
    points=np.array([[200,0,0]])
    center=np.array([0,0,0])
     
    directions=unitize(np.array([[1,1,0]]))
